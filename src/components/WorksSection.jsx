...
export default function WorksSection() {
  ...
  return (
    <section id="works" className="py-14 sm:py-16 md:py-20 px-6 md:px-12 lg:px-16 bg-white text-black border-b border-black/10 relative select-none">
        ...
        <div className="flex items-center justify-end border-b border-black/10 pb-3">
          <div className="flex items-center space-x-2.5">
            <button
              ...
              className="w-9 h-9 rounded-none border border-black/20 bg-white/70 backdrop-blur-md hover:bg-black hover:border-black hover:text-white flex items-center justify-center text-black transition-all duration-300 shadow-sm active:scale-95 cursor-pointer"
            >
              <ChevronLeft size={18} />
            </button>
            <button
              onClick={() => handleScroll('right')}
              className="w-9 h-9 rounded-none border border-black bg-black hover:bg-black/85 hover:backdrop-blur-md text-white flex items-center justify-center transition-all duration-300 shadow-sm active:scale-95 cursor-pointer"
            >
                  ...
                  >
                    ...
                    <div className="relative aspect-[3/4.1] rounded-none overflow-hidden shadow-sm hover:shadow-xl transition-all duration-500 hover:scale-[1.02] border border-black/15 bg-black">
                      ...
                      <div className="absolute top-3 left-3 right-3 flex items-center justify-between">
                        <span className="bg-white/90 backdrop-blur-md px-2.5 py-0.5 rounded-none text-[10px] font-mono-spec font-bold text-black uppercase tracking-wider border border-black/20">
                          {project.category} • <span className="text-black font-extrabold">{project.year}</span>
                        </span>
                      </div>
                      ...
                      <div className="absolute bottom-0 left-0 right-0 p-4 flex flex-col justify-end space-y-2 z-10">
                        <div className="flex items-end justify-between space-x-2">
                          ...
                          <div className="w-8 h-8 rounded-none bg-white/20 backdrop-blur-md flex items-center justify-center text-white shrink-0 group-hover:bg-black transition-all duration-300">
                  ...
                  <button
                    ...
                    className={`h-9 sm:h-10 px-4 font-bold uppercase tracking-wider rounded-none border transition-all duration-300 cursor-pointer whitespace-nowrap ${
                      isActive
                        ...
                        : 'bg-white/60 backdrop-blur-md border-black/20 text-black hover:bg-black/90 hover:border-black hover:text-white hover:shadow-md'
                    }`}
                  >
                    {cat.label}
                  </button>
      ...
      <AnimatePresence>
        {activeModalProject && (
          <motion.div
            ...
            className="fixed inset-0 z-50 bg-black/50 backdrop-blur-md flex items-center justify-center p-4 sm:p-6 lg:p-8"
          >
            <motion.div
              ref={modalContentRef}
              tabIndex={0}
              ...
              className="relative w-full max-w-4xl max-h-[85vh] overflow-y-auto bg-white border border-black/20 p-6 sm:p-8 lg:p-10 my-auto text-black space-y-6 sm:space-y-8 shadow-2xl rounded-none focus:outline-none scroll-smooth"
            >
              {/* Modal Close Button */}
              <button
                onClick={() => setActiveModalProject(null)}
                className="absolute top-5 right-5 sm:top-6 sm:right-6 flex items-center space-x-2 text-zinc-600 hover:text-black transition-colors font-mono-spec text-xs uppercase cursor-pointer z-10"
              >
                <span>CLOSE CASE STUDY</span>
                <div className="w-8 h-8 rounded-none border border-black/20 bg-zinc-100 flex items-center justify-center text-black">
                  <X size={16} />
                </div>
              </button>
              ...
              <div className="space-y-2 pt-2 border-b border-black/10 pb-5">
                <div className="flex items-center space-x-3 font-mono-spec text-xs text-black font-bold">
                ...
                <h2 className="font-display text-2xl sm:text-4xl lg:text-5xl font-black uppercase text-black">
              ...
              <div className="grid grid-cols-1 md:grid-cols-12 gap-6 sm:gap-8 font-sans">
                ...
                <div className="md:col-span-5 bg-zinc-50 p-5 sm:p-6 border border-black/10 space-y-3 rounded-none">
                  <h4 className="text-xs text-black font-mono-spec font-bold uppercase tracking-widest">
              ...
              <div className="pt-6 border-t border-black/10 flex justify-end">
                <button
                  onClick={() => setActiveModalProject(null)}
                  className="flex items-center space-x-2 bg-black hover:bg-zinc-800 text-white px-6 py-3 rounded-none font-sans text-xs font-bold uppercase tracking-wider transition-all cursor-pointer shadow-sm"
                >
                  <span>CLOSE CASE STUDY</span>
                  <X size={16} />
                </button>
